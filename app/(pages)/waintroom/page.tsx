...
export default function WaitingRoomPage() {
  ...
  useEffect(() => {
    ...
    socket.on("player_joined", (payload: { players: Array<{ userId: string; userName: string }> }) => {
      const mapped = (payload?.players || []).map(p => ({ id: p.userId, name: p.userName }));
      setPlayers(mapped);
    });

    socket.on("game_state", (game: any) => {
      const mapped = (game?.players || []).filter((p: any) => p.isConnected).map((p: any) => ({ id: p.userId, name: p.userName }));
      setPlayers(mapped);
    });
  ...
  const handleShareLink = async () => {
    const base = `${window.location.origin}/waintroom?room=${encodeURIComponent(room)}`;
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-gray-400 mt-1">Partagez le code pour inviter des joueurs.</p>
              <p className="text-gray-400 mt-1">Statut: <span className={status === "connected" ? "text-emerald-400" : status === "error" ? "text-red-400" : "text-yellow-400"}>{status}</span></p>
            </div>
            <button
              onClick={handleLeave}
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-black/30 border border-white/10 rounded-xl p-5">
              ...
              <div className="text-3xl font-semibold tracking-widest text-center py-4 select-all">{room}</div>
              <button
              ...
              <div className="mt-3 text-sm text-gray-400 text-center">Max joueurs: {maxPlayers}</div>
            </div>
            ...
            <div className="bg-black/30 border border-white/10 rounded-xl p-5">
              ...
              <div className="space-y-3">
                {players.length === 0 && (
                  <div className="text-gray-400 text-sm">En attente de joueurs...</div>
                )}
                {players.map((p) => (
                  <div key={p.id} className="flex items-center justify-between bg-white/5 rounded-lg px-3 py-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-medium">{p.name || `Joueur ${p.id}`}</div>
